...
import app.core.logging_config  # configura o logging antes de carregar os endpoints

from fastapi import FastAPI, UploadFile, File, HTTPException
from fastapi.middleware.cors import CORSMiddleware
from pydantic import BaseModel, Field, HttpUrl

from app.core.config import ALLOWED_ORIGINS, COLLECTION_NAME
from app.database.seed import seed_documents
from app.rag.generation import query_rag
from app.rag.ingest import ensure_collection_exists, ingest_document, ingest_from_url, qdrant
...
async def lifespan(app: FastAPI):
    ...
    await seed_documents()  # idempotente: IDs determinísticos evitam duplicatas
    yield
...
# Defina ALLOWED_ORIGINS em produção; sem ela, somente desenvolvimento usa '*'.
app.add_middleware(
    CORSMiddleware,
...
async def ingest_from_url_endpoint(request: IngestUrlRequest):
    ...
    """
    try:
        result = await ingest_from_url(request.url)
    except Exception as e:
